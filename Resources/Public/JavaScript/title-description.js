...
class TitleDescription {
    ...
    initializeProgressBars() {
        if (this.progressBarsInitialized || !store.getState().content)
            return;
        ...
        progressBarItems.forEach(({ input, storeKey, defaultValue }) => {
            ...
            input.addEventListener("input", new DebounceEvent("input", () => {
                let value = input.value;
                if (storeKey === "title") {
                    const pageTitleValue = FormEngine.getInputElementValue("pageTitle");
                    if (value === "") {
    ...
    addProgressBar(input, storeKey, defaultValue) {
    ...
    initializeSeoTabLoad() {
        document.querySelectorAll(`li.t3js-tabmenu-item`).forEach((item) => {
            ...
            new DebounceEvent("click", (e) => {
                ...
                FormEngine.getElement("title")?.setAttribute("placeholder", pageTitleValue);
                if (value === "") {
                    value = pageTitleValue;
                }
                value = this.getTitleValue(value);
